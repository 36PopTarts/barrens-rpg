	...
	if(Player::isExposed(%this) && %object != -1 && %type != $NullDamageType && !Player::IsDead(%this))
	{
		...
		if(!IsDead(%this))
		{
			...
			if(%msgcolor != "")
			{
				if(%type != $SpellDamageType)
				{
					newprintmsg(%shooterClient,"You try to "@%hitpresent@" <f1>" @ rpg::getname(%damagedClient) @ "<f0>, but miss!", %msgcolor);

					%time = getIntegerTime(true) >> 5;
					if(%time - %damagedClient.lastMissMessage > 2)
					{
						%damagedClient.lastMissMessage = %time;
						newprintmsg(%damagedClient,"<f1>"@%hitby @ "<f0> tries to "@%hitpresent@" you, but misses!", %msgcolor);
					}
				}
				else
				{
					newprintmsg(%shooterClient, "<f1>"@rpg::getname(%damagedClient) @ "<f0> resists your spell!", %msgcolor);
					newprintmsg(%damagedClient, "You resist <f1>" @ %hitby @ "<f0>'s spell!", %msgcolor);
				}
			}
			...
			if(%value)
			{
				...
				if(%convValue > 0)
				{
					...
					//--------------------
					if(%shooterClient != %damagedClient)
						newprintmsg(%shooterClient, "You " @ %saction @ " <f1>" @ rpg::getname(%damagedClient) @ "<f0> - <f2>" @ %convValue @ "<f0> points", $MsgRed);
					newprintmsg(%damagedClient, "You were " @ %daction @ " by <f1>" @ %hitby @ "<f0> - <f2>" @ %convValue @ "<f0> points", $MsgRed);
					...
					else
					{
						if(fetchData(%shooterClient, "invisible"))
							...
							%sname = Client::getName(%shooterClient);
						%dname = Client::getName(%damagedClient);
					}					%hitpresentrad = %hitpresent;					if(%hitpresentrad == "slashs")						%hitpresentrad = "slashes";
					radiusAllExcept(%damagedClient, %shooterClient, %sname @ " "@%hitpresentrad@"s " @ %dname @ " for " @ %convValue @ " points of damage!", true);
				}
				else if(%convValue < 0)
				{
					...
					newprintmsg(%shooterClient, "You try to "@%hitpresent@" <f1>" @ rpg::getname(%damagedClient) @ "<f0>, but miss! (LCK)", $MsgRed);
					newprintmsg(%damagedClient, "<f1>"@%hitby @ "<f0> tries to "@%hitpresent@" you, but misses! (LCK)", $MsgRed);
				}
